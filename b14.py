...
trading_hours = {
            ...
            3: (datetime.time(15, 0, 0),  datetime.time(5, 0, 0))}              # 夜盤
# --------------------------------------------------------------------------------------------------------------------------------
class AppFrame(wx.Frame):
    ...
    def logon(self,event):
        ...
        YTquote.api_Logon(acc0, pwd0, port, reqType)                            
        self.f_logon  = True
        # -----------------------------------------------------------------------------------------登入後才能做的事情
    ...
    def RegisterQuoteSymbol(self, symbol):
        ...
        YTquote.api_Register(symbol , reqType)
        time.sleep(1.5)             
    ...
    def SetRegister_Symbol(self):
        # 取得並註冊 (小台指-當月) 商品代號 
        if  self.Symbol0 != '':
            YTquote.api_UnRegister(self.Symbol0)                                             # 商品代號註銷
            time.sleep(1.5)         
    ...
    def OnGetMktAll(self,symbol, RefPri, OpenPri, HighPri, LowPri, UpPri, DnPri, MatchTime, MatchPri, MatchQty, TolMatchQty,BestBuyQty, BestBuyPri, BestSellQty,BestSellPri, FDBPri, FDBQty, FDSPri, FDSQty, ReqType):
        ...
        if (self.Symbol0 != symbol):                                                    # 商品代號不符合
            return
        time_period = get_time_period(MatchTime)
        if (time_period != 1) and (time_period !=3):                                    # 不在開盤時段
            return
        i_tqty = int(TolMatchQty) if TolMatchQty != '' else 0                           # 總量
        i_qty  = int(MatchQty)    if MatchQty    != '' else 0                           # 單量
...
if __name__ == "__main__":
    ...
    frame.Show(True)
    YTquote = api_utils.YuantaQuoteAXCtrl(frame)
    app.MainLoop()
